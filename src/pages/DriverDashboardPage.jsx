...
export const DriverDashboardPage = ({ setActiveTab }) => {
  ...
  return (
    <div className="space-y-8 pb-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
      ...
      {/* SUB-SCREEN (d): DRIVER PROFILE DETAILS */}
      {driverTab === 'profile' && (
        <div className="bg-white rounded-3xl p-8 border border-slate-200 shadow-card max-w-3xl space-y-6">
          <div>
            <h2 className="text-xl font-extrabold text-slate-900">Driver Credentials & Telematics Profile</h2>
            <p className="text-xs text-slate-500">Official log of registered driver status and vehicle dispatch credentials.</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm">
            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-150 space-y-1">
              ...
              <p className="font-extrabold text-slate-900">{currentUser?.name || driverInfo.name}</p>
            </div>

            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-150 space-y-1">
              <span className="text-xs text-slate-400 font-bold uppercase">Email Address</span>
              <p className="font-extrabold text-slate-900">{currentUser?.email || driverInfo.email}</p>
            </div>

            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-150 space-y-1">
              <span className="text-xs text-slate-400 font-bold uppercase">Phone Number</span>
              <p className="font-extrabold text-slate-900">{currentUser?.phone || driverInfo.phone || '+65 9112 3456'}</p>
            </div>

            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-150 space-y-1">
              <span className="text-xs text-slate-400 font-bold uppercase">Driver License Number</span>
              <p className="font-extrabold text-slate-900">{currentUser?.licenseNumber || 'S9876543A'}</p>
            </div>

            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-150 space-y-1">
              <span className="text-xs text-slate-400 font-bold uppercase">Date of Birth</span>
              <p className="font-extrabold text-slate-900">{currentUser?.dob || '1990-05-12'}</p>
            </div>

            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-150 space-y-1">
              <span className="text-xs text-slate-400 font-bold uppercase">Assigned Vehicle</span>
              <p className="font-extrabold text-slate-900">{driverInfo.vehicleType} ({driverInfo.vehicleId})</p>
            </div>

            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-150 space-y-1">
              <span className="text-xs text-slate-400 font-bold uppercase">Assigned Hub Terminal</span>
              <p className="font-extrabold text-slate-900">{driverInfo.assignedHub}</p>
            </div>

            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-150 space-y-1">
              <span className="text-xs text-slate-400 font-bold uppercase">Duty Status</span>
            ...
            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-150 space-y-1">
              <span className="text-xs text-slate-400 font-bold uppercase">SLA Reliability Rating</span>
              <p className="font-extrabold text-slate-900">⭐ {driverInfo.rating} / 5.0 ({driverInfo.onTimeRate} On-Time)</p>
            </div>

            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-150 space-y-1">
              <span className="text-xs text-slate-400 font-bold uppercase">Completed Shipments</span>
              <p className="font-extrabold text-slate-900">{driverInfo.deliveriesCompleted} deliveries</p>
            </div>
          </div>
        </div>
      )}
