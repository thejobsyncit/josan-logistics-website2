...
export const BookShipmentPage = ({ setActiveTab }) => {
  const { addShipment, setSelectedInvoiceShipment, showToast, currentUser } = useLogistics();
  ...
  // Shipment References Handlers
  const handleAddReference = () => {
    setFormData(prev => ({
      ...prev,
      shipmentReferences: [...(prev.shipmentReferences || ['']), '']
    }));
  };

  const handleReferenceChange = (index, value) => {
    const updated = [...(formData.shipmentReferences || [''])];
    updated[index] = value;
    setFormData(prev => ({ ...prev, shipmentReferences: updated }));
  };

  const handleRemoveReference = (index) => {
    if ((formData.shipmentReferences || []).length <= 1) return;
    const updated = (formData.shipmentReferences || []).filter((_, i) => i !== index);
    setFormData(prev => ({ ...prev, shipmentReferences: updated }));
  };
  ...
  const validateStep = (stepNumber) => {
    ...
    if (stepNumber === 1) {
      ...
      if (formData.senderIsBusiness && !formData.senderCompany.trim()) {
      ...
      if (!formData.senderAddress1.trim()) {
      ...
      if (!formData.senderState.trim()) {
        showToast('Please enter Sender State', 'warning');
        return false;
      }
      ...
      const senderReqLen = getPhoneLength(formData.senderCountryCode);
      if (!formData.senderPhone || !formData.senderPhone.trim()) {
        showToast('Please enter Sender Phone Number', 'warning');
        return false;
      }
      if (formData.senderPhone.trim().length !== senderReqLen) {
        showToast(`Sender Phone Number must be exactly ${senderReqLen} digits for ${formData.senderCountryCode}`, 'warning');
        return false;
      }
      ...
      if (formData.receiverIsBusiness && !formData.receiverCompany.trim()) {
      ...
      if (!formData.receiverAddress1.trim()) {
        showToast('Please enter Recipient Address Line 1', 'warning');
        return false;
      }
      if (!formData.receiverPostalCode.trim()) {
      ...
      if (!formData.receiverState.trim()) {
        showToast('Please enter Recipient State', 'warning');
        return false;
      }
      if (!formData.receiverEmail.trim() || !formData.receiverEmail.includes('@')) {
        showToast('Please enter a valid Recipient Email Address', 'warning');
        return false;
      }
      const receiverReqLen = getPhoneLength(formData.receiverCountryCode);
      if (!formData.receiverPhone || !formData.receiverPhone.trim()) {
        showToast('Please enter Recipient Phone Number', 'warning');
        return false;
      }
      if (formData.receiverPhone.trim().length !== receiverReqLen) {
        showToast(`Recipient Phone Number must be exactly ${receiverReqLen} digits for ${formData.receiverCountryCode}`, 'warning');
        return false;
      }
      return true;
    }
    ...
    if (stepNumber === 2) {
      ...
      const firstRef = (formData.shipmentReferences || [''])[0];
      if (!firstRef || !firstRef.trim()) {
        showToast('Primary Shipment Reference is required (appears on waybill label)', 'warning');
        return false;
      }
  ...
  const handleStepPillClick = (targetStep) => {
    ...
    for (let s = 1; s < targetStep; s++) {
      if (!validateStep(s)) {
        setShowValidationErrors(true);
        setCurrentStep(s);
        showToast(`Please complete mandatory fields in Step ${s} before proceeding`, 'warning');
        return;
      }
    }
    setShowValidationErrors(false);
  ...
  return (
    <div className="space-y-8 pb-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 animate-fade-in">
      ...
      <div className="bg-white rounded-3xl p-8 border border-slate-200 shadow-card flex flex-col md:flex-row items-center justify-between gap-6">
        <div>
          <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-orange-50 text-orange-600 text-xs font-bold mb-2 border border-orange-200">
            ...
            <span>Create New Freight Shipment</span>
          </div>
          ...
          {steps.map((step) => (
            ...
            >
              <span className={`w-5 h-5 rounded-full text-[11px] flex items-center justify-center font-bold ${
                currentStep === step.number ? 'bg-white text-orange-600' : 'bg-slate-200 text-slate-700'
              }`}>
                {step.number}
              </span>
      ...
      {/* STEP 1: SENDER & RECIPIENT ADDRESS DETAILS (SIDE-BY-SIDE FROM / TO) */}
      {currentStep === 1 && (
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center space-x-2">
              <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Create Shipment</span>
              <span className="text-slate-300">—</span>
              <span className="text-xs font-bold text-slate-400">Pay</span>
              <span className="text-slate-300">—</span>
              <span className="text-xs font-bold text-slate-400">Print</span>
            </div>
          ...
          {/* Side-by-side From & To Address Cards */}
          <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-8">
            
            {/* Central Switch Button (Desktop Floating Pill) */}
            <div className="hidden lg:flex absolute left-1/2 top-4 -translate-x-1/2 z-20">
              <button
                type="button"
                onClick={handleSwapAddresses}
                className="px-3.5 py-1.5 bg-slate-900 hover:bg-slate-800 text-orange-400 hover:text-white rounded-full text-xs font-black shadow-lg border-2 border-white transition-all flex items-center space-x-1.5 cursor-pointer hover:scale-105 active:scale-95 group"
                title="Swap From and To Addresses"
              >
                <ArrowRightLeft className="w-3.5 h-3.5 text-orange-400 group-hover:rotate-180 transition-transform duration-300" />
                <span className="whitespace-nowrap">Switch</span>
              </button>
            </div>

            {/* FROM (SENDER) COLUMN */}
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-card space-y-4 text-xs text-left">
              
              {/* Header */}
              <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                <div className="flex items-center space-x-3">
                  <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-orange-500 to-amber-600 text-white flex items-center justify-center shadow-md shadow-orange-500/20 ring-4 ring-orange-50">
                    <MapPin className="w-5 h-5" />
                  </div>
                  <div>
                    <div className="flex items-center space-x-2">
                      <span className="px-2 py-0.5 bg-orange-100 text-orange-700 font-black text-[10px] rounded-md uppercase tracking-wider">
                        From
                      </span>
                ...
                {formData.senderName && formData.senderAddress1 && (
                  <span className="flex items-center space-x-1 text-[11px] font-bold text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-full border border-emerald-200 shadow-xs">
              ...
              <div className="flex items-center space-x-2">
                ...
                />
                <label htmlFor="senderBusinessCheck" className="font-bold text-slate-800 cursor-pointer">
                  Business Contact
                </label>
              </div>
              ...
              {/* Company */}
              <div>
                <label className="block font-bold text-slate-700 uppercase tracking-wider text-[10px] mb-1">Company *</label>
                <div className="relative flex items-center">
                  <input
                    type="text"
              ...
              <div>
                ...
                <div className="relative mb-2">
                  ...
                  {formData.senderAddress1 && formData.senderAddress1.trim() ? (
                    ...
                    <span className="text-rose-500 font-bold text-xs absolute right-3 top-3 pointer-events-none">*</span>
                  )}
                </div>
                <div className="relative mb-2">
                  <input
              ...
              <div className="flex items-center space-x-2 pt-1">
                <input
                  type="checkbox"
                  id="senderResidentialCheck"
                  checked={formData.senderIsResidential}
                  onChange={(e) => setFormData({ ...formData, senderIsResidential: e.target.checked })}
                  className="w-4 h-4 text-orange-500 rounded focus-orange cursor-pointer"
                />
                <label htmlFor="senderResidentialCheck" className="font-bold text-slate-700 cursor-pointer">
                  Residential Address
                </label>
              </div>
              ...
              {/* Phone Type | Code | Phone */}
              <div>
                <div className="grid grid-cols-12 gap-2">
                  <div className="col-span-4">
                    <label className="block font-bold text-slate-700 uppercase tracking-wider text-[10px] mb-1">Phone Type</label>
                    <select
                  ...
                  <div className="col-span-3">
                    <label className="block font-bold text-slate-700 uppercase tracking-wider text-[10px] mb-1">Code</label>
                    <select
                      ...
                      return (
                        <div className="relative flex items-center">
                          <input
                            ...
                            placeholder={`e.g. ${'9'.repeat(reqLen)} (${reqLen} digits)`}
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  className="w-4 h-4 text-orange-500 rounded focus-orange cursor-pointer"
                />
                <label htmlFor="senderSmsCheck" className="font-bold text-slate-700 cursor-pointer">
                  SMS Enabled
                </label>
              </div>
              ...
              <div className="pt-2 border-t border-slate-100 space-y-3">
                ...
                <div className="relative">
                  ...
                  {formData.senderTaxIdNumber && formData.senderTaxIdNumber.trim() && (
                    <span className="text-emerald-600 font-extrabold text-sm absolute right-3 top-8 pointer-events-none">✓</span>
                  )}
                </div>
              </div>
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-card space-y-4 text-xs text-left">
              ...
              <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                <div className="flex items-center space-x-3">
                  <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 text-white flex items-center justify-center shadow-md shadow-emerald-500/20 ring-4 ring-emerald-50">
                    <Navigation className="w-5 h-5 rotate-45" />
                  </div>
                  <div>
                    <div className="flex items-center space-x-2">
                      <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 font-black text-[10px] rounded-md uppercase tracking-wider">
                        To
                      </span>
              ...
              <div className="flex items-center space-x-2">
                <input
                  type="checkbox"
                  id="receiverBusinessCheck"
                  checked={formData.receiverIsBusiness}
                  onChange={(e) => setFormData({ ...formData, receiverIsBusiness: e.target.checked })}
                  className="w-4 h-4 text-orange-500 rounded focus-orange cursor-pointer"
                />
                <label htmlFor="receiverBusinessCheck" className="font-bold text-slate-800 cursor-pointer">
                  Business Contact
                </label>
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className={`w-full p-2.5 bg-slate-50 border-2 rounded-xl font-bold text-slate-900 cursor-pointer transition-all ${getInputClass(formData.receiverCountry, true)}`}
                >
                  {countryOptions.map(c => (
                    <option key={c} value={c}>{c}</option>
                  ))}
              ...
              <div className="flex items-center space-x-2 pt-1">
                <input
                  type="checkbox"
                  id="receiverResidentialCheck"
                  checked={formData.receiverIsResidential}
                  onChange={(e) => setFormData({ ...formData, receiverIsResidential: e.target.checked })}
                  className="w-4 h-4 text-orange-500 rounded focus-orange cursor-pointer"
                />
                <label htmlFor="receiverResidentialCheck" className="font-bold text-slate-700 cursor-pointer">
                  Residential Address
                </label>
              </div>
                      ...
                      return (
                        <div className="relative flex items-center">
                          <input
                            ...
                            placeholder={`e.g. ${'8'.repeat(reqLen)} (${reqLen} digits)`}
              ...
              <div className="flex items-center space-x-2">
                <input
                  ...
                  className="w-4 h-4 text-orange-500 rounded focus-orange cursor-pointer"
                />
                <label htmlFor="receiverSmsCheck" className="font-bold text-slate-700 cursor-pointer">
                  SMS Enabled
                </label>
              </div>
              ...
              <div className="pt-2 border-t border-slate-100 space-y-3">
                <div>
                  ...
                  >
                    <option value="GSTIN">GSTIN / Business Tax ID</option>
                    <option value="PAN">Personal ID / PAN</option>
                    <option value="NRIC">NRIC / FIN</option>
                    <option value="Passport">Passport Number</option>
                  </select>
                </div>
          ...
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
            ...
            <button
              ...
              className="px-8 py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-extrabold text-sm shadow-emerald-sm transition-all flex items-center space-x-2 cursor-pointer active:scale-95"
            >
              <span>Next</span>
      ...
      {currentStep === 2 && (
        <div className="space-y-6 animate-fade-in text-left">
          ...
          {/* Top Address Summary Bar (From & To Overview with Edit Button) */}
          <div className="bg-slate-100/90 rounded-2xl p-5 border border-slate-200 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 w-full md:w-auto text-xs">
              
              {/* FROM Summary */}
              <div className="flex items-start space-x-3">
                <div className="w-7 h-7 rounded-full bg-slate-400 text-white font-black flex items-center justify-center text-xs shrink-0 mt-0.5 shadow-xs">
                  A
                </div>
                <div className="space-y-0.5 text-slate-600">
                  <span className="font-extrabold text-slate-500 text-[11px] block uppercase tracking-wider">From</span>
                  <p className="font-extrabold text-slate-900 text-sm leading-tight">{formData.senderName || 'Sender Name'}</p>
                  {formData.senderCompany && <p className="font-semibold text-slate-700">{formData.senderCompany}</p>}
                  <p>{formData.senderAddress1 || 'Address Line 1'}{formData.senderAddress2 ? `, ${formData.senderAddress2}` : ''}</p>
                  <p className="font-bold text-slate-800">{formData.senderCity || 'City'}, {formData.senderState || 'State'} {formData.senderPostalCode || 'Postal'}</p>
                  <p className="font-semibold text-slate-700">{formData.senderCountry}</p>
                </div>
              </div>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div className="space-y-0.5 text-slate-600">
                  ...
                  <p className="font-extrabold text-slate-900 text-sm leading-tight">{formData.receiverName || 'Recipient Name'}</p>
                  {formData.receiverCompany && <p className="font-semibold text-slate-700">{formData.receiverCompany}</p>}
                  <p>{formData.receiverAddress1 || 'Address Line 1'}{formData.receiverAddress2 ? `, ${formData.receiverAddress2}` : ''}</p>
                  <p className="font-bold text-slate-800">{formData.receiverCity || 'City'}, {formData.receiverState || 'State'} {formData.receiverPostalCode || 'Postal'}</p>
                  <p className="font-semibold text-slate-700">{formData.receiverCountry}</p>
                </div>
              </div>
            ...
            <div className="self-end md:self-center shrink-0">
              <button
                ...
                className="px-4 py-2 bg-white hover:bg-slate-50 text-slate-800 font-extrabold text-xs rounded-xl border border-slate-300 shadow-xs flex items-center space-x-1.5 transition-all cursor-pointer hover:border-slate-400 active:scale-95"
              >
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-card space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-8 space-y-8">
                ...
                {/* Invoice Value Card (Image 1) */}
                <div className="space-y-4 pt-4 border-t border-slate-100">
                  <div>
                    <h3 className="text-base font-extrabold text-slate-900">Invoice Value</h3>
                    <p className="text-xs font-semibold text-slate-500 mt-0.5">
                      Include any additional charges for this shipment.
                    </p>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-stretch">
                    {/* Left Box: Goods Value & Add Charges */}
                    <div className="md:col-span-8 bg-slate-50/80 rounded-xl p-4 border border-slate-200 space-y-3 flex flex-col justify-between">
                      <div className="flex items-center justify-between text-xs font-semibold text-slate-700">
                        <span>Total value for all goods/items only (excluding other charges)</span>
                        <div className="flex items-center space-x-1 font-mono font-extrabold text-slate-900">
                          <span>{totalValueInr > 0 ? totalValueInr.toFixed(2) : '--.--'}</span>
                          <span className="text-slate-500 font-bold">INR</span>
                        </div>
                      </div>

                      {showAdditionalCharges && (
                        <div className="pt-2 border-t border-slate-200/60 space-y-2 animate-fade-in">
                          <div className="flex items-center space-x-2">
                            <div className="relative flex-1 flex items-center">
                              <input
                                type="text"
                                value={formData.additionalChargesDesc || 'Freight & Handling'}
                                onChange={(e) => setFormData({ ...formData, additionalChargesDesc: e.target.value })}
                                placeholder="Charge Description (e.g. Insurance, Packing)"
                                className={`w-full p-2 bg-white border-2 rounded text-xs font-bold text-slate-900 pr-7 transition-all ${getInputClass(formData.additionalChargesDesc, false)}`}
                              />
                              {formData.additionalChargesDesc && formData.additionalChargesDesc.trim() && (
                                <span className="text-emerald-600 font-extrabold text-xs absolute right-2 pointer-events-none">✓</span>
                              )}
                            </div>
                            <div className="relative flex items-center">
                              <input
                                ...
                                placeholder="0.00"
                                className={`w-28 p-2 bg-white border-2 rounded text-xs font-mono font-bold text-slate-900 pr-6 transition-all ${getInputClass(formData.additionalCharges, false)}`}
                              />
                              {formData.additionalCharges && formData.additionalCharges.toString().trim() && (
                                <span className="text-emerald-600 font-extrabold text-xs absolute right-1.5 pointer-events-none">✓</span>
                              )}
                            </div>
                      ...
                      <div>
                        <button
                          ...
                          className="px-3 py-1.5 bg-orange-500 hover:bg-orange-600 text-white font-extrabold text-xs rounded transition-all shadow-xs flex items-center space-x-1.5 cursor-pointer border border-orange-600 active:scale-95"
                        >
                          <span>{showAdditionalCharges ? 'Remove Charges' : 'Add Charges'}</span>
                        </button>
                      </div>
                    </div>

                    {/* Right Box: Total Invoice Value */}
                    <div className="md:col-span-4 bg-white rounded-xl p-4 border border-slate-300 shadow-xs flex flex-col justify-center space-y-1">
                      <h4 className="font-extrabold text-slate-900 text-xs">Total Invoice Value</h4>
                      <p className="text-[11px] text-slate-500 font-medium">(for customs purposes)</p>
                      <div className="font-mono font-extrabold text-base text-slate-900 pt-1">
                        {(totalValueInr + (parseFloat(formData.additionalCharges) || 0)) > 0
                          ? (totalValueInr + (parseFloat(formData.additionalCharges) || 0)).toFixed(2)
                          : '--.--'}{' '}
                        <span className="text-xs font-bold text-slate-600">INR</span>
                      </div>
                    </div>
                ...
                {/* Pre-calculated Duties & Taxes Checkbox (Image 1) */}
                <div className="pt-2">
                  <div className="flex items-center space-x-2">
                    <input
                      type="checkbox"
                      id="precalcDutiesCheck"
                      checked={formData.includePrecalculatedDuties || false}
                      onChange={(e) => setFormData({ ...formData, includePrecalculatedDuties: e.target.checked })}
                      className="w-4 h-4 text-orange-500 rounded focus-orange cursor-pointer"
                    />
                    <label htmlFor="precalcDutiesCheck" className="font-bold text-slate-700 text-xs cursor-pointer flex items-center space-x-1">
                      <span>I want to include pre-calculated duties and taxes for this shipment</span>
                      <Info className="w-3.5 h-3.5 text-blue-500 cursor-pointer" title="Customs pre-clearance duty estimate included on waybill" />
                    </label>
                  </div>
                </div>

                {/* Add Shipment References Section (Image 1) */}
                <div className="space-y-3 pt-4 border-t border-slate-100">
                  <div>
                    <h3 className="text-base font-extrabold text-slate-900">Add Shipment References</h3>
                    <p className="text-xs font-semibold text-slate-500 mt-0.5">
                      Your first reference will appear on your shipping label/waybill. All additional references will not show on shipping label/waybill.
                    </p>
                  </div>

                  <div className="space-y-3 max-w-xl">
                    {(formData.shipmentReferences || ['']).map((refVal, idx) => {
                      const isFilled = refVal && refVal.trim().length > 0;
                      return (
                        <div key={idx} className="flex items-center space-x-2">
                          <div className="relative flex-1">
                            <input
                              ...
                              placeholder={idx === 0 ? "Reference (appears on shipping label/waybill)" : `Additional Reference #${idx + 1}`}
                              className={`w-full p-2.5 bg-white border-2 rounded font-semibold text-xs text-slate-900 shadow-xs pr-8 transition-all ${getInputClass(refVal, idx === 0)}`}
                              required={idx === 0}
                            />
                            {isFilled ? (
                              <span className="absolute right-3 top-2.5 text-emerald-600 font-extrabold text-sm leading-none flex items-center justify-center pointer-events-none">
                                ✓
                              </span>
                            ) : (
                              <span className="absolute right-3 top-2.5 text-rose-500 font-bold text-xs leading-none pointer-events-none">
                                *
                              </span>
                            )}
                          </div>
                          {idx > 0 && (
                            <button
                              ...
                              className="p-2 text-rose-500 hover:text-rose-700 hover:bg-rose-50 rounded transition-colors cursor-pointer"
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 bg-orange-500 hover:bg-orange-600 text-white font-extrabold text-xs rounded transition-all shadow-xs flex items-center space-x-1.5 cursor-pointer active:scale-95 border border-orange-600"
                    >
            ...
            {/* Footer Navigation (Image 2 - Green Next Button) */}
            <div className="pt-6 border-t border-slate-100 flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={handleNextStep}
                className="px-8 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-extrabold text-sm shadow-sm transition-all flex items-center space-x-2 cursor-pointer active:scale-95"
              >
                <span>Next</span>
                <ChevronRight className="w-4 h-4" />
              </button>
      ...
      {currentStep === 3 && (
        <div className="space-y-6 animate-fade-in text-left">
          ...
          {/* Top Bar with Stepper & Actions (Create Shipment — Pay — Print | Cancel | Save for Later) */}
          <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center space-x-2">
              <span className="text-xs font-black text-slate-900 uppercase tracking-wider">Create Shipment</span>
              <span className="text-slate-300">—</span>
              <span className="text-xs font-bold text-slate-400">Pay</span>
              <span className="text-slate-300">—</span>
              <span className="text-xs font-bold text-slate-400">Print</span>
