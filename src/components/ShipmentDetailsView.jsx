...
export const ShipmentDetailsView = ({ shipment: propShipment, onClose }) => {
  ...
  const downloadPodPdf = () => {
    try {
      ...
      doc.save(`Josan_POD_${shipment.id}.pdf`);
                ...
                {trackingStages.map((stage, idx) => {
                  ...
                  return (
                    <div key={stage.key} className="flex flex-col items-center text-center relative z-10 w-28">
                      ...
                      <span className={`text-xs font-bold mt-2.5 ${isCurrent ? 'text-orange font-outfit' : isCompleted ? 'text-[#10182D]' : 'text-slate-400'}`}>
                        {stage.label}
                      </span>
                  ...
                  <div key={stage.key} className="flex items-start gap-3 relative">
                    {idx < trackingStages.length - 1 && (
                      <div className={`absolute left-4 top-8 bottom-0 w-0.5 -ml-px ${
                        idx < currentStageIdx ? 'bg-orange' : 'bg-slate-200'
                      }`} />
                    )}
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs shrink-0 transition-all ${
                      ...
                      {idx < currentStageIdx ? <CheckCircle2 size={16} /> : idx + 1}
                    </div>
                    <div className="flex-1 pt-0.5">
                      <div className="flex items-center justify-between">
                        <span className={`text-sm font-bold ${isCurrent ? 'text-orange' : isCompleted ? 'text-[#10182D]' : 'text-slate-400'}`}>
                          {stage.label}
                        </span>
                        {isCurrent && (
                          <span className="text-[10px] uppercase font-bold bg-orange text-white px-2 py-0.5 rounded-full">
                            Current Status
                          </span>
                        )}
                      </div>
